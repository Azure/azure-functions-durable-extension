 ...
 *
 * View runtime logs:
 *   - Locally at C:\Users\{user}\AppData\Local\Temp\LogFiles\Application\Functions
 *   - In the Azure Portal if App Insights instrumentation key configured in local.settings.json
 *     (see https://docs.microsoft.com/en-us/azure/azure-functions/functions-app-settings)
 */
namespace VSSample
{
    public static class Monitor
    {
        ...
        public static async Task Run([OrchestrationTrigger] DurableOrchestrationContext monitorContext, TraceWriter log)
        {
            ...
            log.Info($"Received monitor request. Location: {input.Location}. Phone: {input.Phone}.");

            VerifyRequest(input);

            DateTime endTime = monitorContext.CurrentUtcDateTime.AddHours(6);
            log.Info($"Instantiating monitor for {input.Location}. Expires: {endTime}.");

            while (monitorContext.CurrentUtcDateTime < endTime)
            {
                // Check the weather
                log.Info($"Checking current weather conditions for {input.Location} at {monitorContext.CurrentUtcDateTime}.");
                bool isClear = await monitorContext.CallActivityAsync<bool>("E3_GetIsClear", input.Location);

                if (isClear)
                {
                    // It's not raining! Or snowing. Or misting. Tell our user to take advantage of it.
                    log.Info($"Detected clear weather for {input.Location}. Notifying {input.Phone}.");
                ...
                else
                {
                    ...
                    log.Info($"Next check for {input.Location} at {nextCheckpoint}.");
    ...
    internal class WeatherUnderground
    {
        ...
        internal static async Task<WeatherCondition> GetCurrentConditionsAsync(Location location)
        {
            ...
            var toString = conditions.ToString();

            JToken currentObservation;
            if (!conditions.TryGetValue("current_observation", out currentObservation))
            {
                var error = conditions.SelectToken("response.error"); //type, description
