...
internal class WeatherUnderground
{
        ...
        Location location)
    {
        var callString = string.Format(
            ...
            Environment.GetEnvironmentVariable("WeatherUndergroundApiKey"), 
            location.State, 
            location.City);
        ...
        if (!conditions.TryGetValue("current_observation", out currentObservation))
        {
            ...
            {
                throw new ArgumentException(@"Could not find weather for this 
                    location. Try being more specific.");
            }
        }
