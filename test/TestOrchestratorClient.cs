...
namespace Microsoft.Azure.WebJobs.Extensions.DurableTask.Tests
{
    internal class TestOrchestratorClient
    {
        ...
        public async Task<DurableOrchestrationStatus> GetStatusAsync(bool showHistory = false, bool showHistoryInputOutput = false)
        {
            DurableOrchestrationStatus status = await this.innerClient.GetStatusAsync(this.instanceId, showHistory, showHistoryInputOutput);
        ...
        public async Task<DurableOrchestrationStatus> WaitForCompletionAsync(TimeSpan timeout, ITestOutputHelper output, bool showHistory = false, bool showHistoryInputOutput = false)
        {
            Stopwatch sw = Stopwatch.StartNew();
            do
            {
                DurableOrchestrationStatus status = await this.GetStatusAsync(showHistory, showHistoryInputOutput);
