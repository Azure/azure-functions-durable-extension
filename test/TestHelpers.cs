    ...
    internal static class TestHelpers
    {
        ...
        private static List<string> GetLogs_OrchestrationEventGridApiReturnBadStatus(string messageId, string[] functionNames)
        {
            var list = new List<string>()
            {
                ...
                $"Error in sending message to the EventGrid. Please check the host.json configuration durableTask.EventGridTopicEndpoint and EventGridKey. LifeCycleNotificationHelper.SendNotificationAsync - Status: InternalServerError Reason Phrase: Internal Server Error For more detail: {messageId}: Function '{functionNames[0]} ({FunctionType.Orchestrator})', function state Started version '' failed with an error. Reason: . IsReplay: False. HubName: OrchestrationStartAndCompleted. AppName: . SlotName: . ExtensionVersion: 1.2.1.0.",
                $"Error in sending message to the EventGrid. Please check the host.json configuration durableTask.EventGridTopicEndpoint and EventGridKey. LifeCycleNotificationHelper.SendNotificationAsync - Status: InternalServerError Reason Phrase: Internal Server Error For more detail: {messageId}: Function '{functionNames[0]} ({FunctionType.Orchestrator})', function state Completed version '' failed with an error. Reason: . IsReplay: False. HubName: OrchestrationStartAndCompleted. AppName: . SlotName: . ExtensionVersion: 1.2.1.0.",
            };
