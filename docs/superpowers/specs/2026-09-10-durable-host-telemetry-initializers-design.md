...
For the internal WebJobs type and the ASP.NET Core client-IP type, match the exact full type name plus the defining assembly's simple name and public-key token. Use the public `ApplicationInsightsLoggerOptions` and `ClientIpHeaderTelemetryInitializer` assemblies as the respective markers. Deliberately ignore assembly version: the Functions host and extension can resolve different supported versions of these private dependencies in separate load contexts, so runtime `Assembly`/`Type` object identity is not reliable. Keep exact runtime type equality for the correlation default in the shared AI core assembly. Do not use `is` checks that also exclude custom subclasses, exclude an entire namespace/assembly, or match names containing "Functions" or "Telemetry". Same-name types in unrelated assemblies remain eligible; this compatibility filter is not a security boundary.

This narrowly depends on an internal WebJobs type name. Add a version-pinned characterization test that obtains the real host registration and proves the unsafe initializer is recognized. Exercise real WebJobs and ASP.NET Core AI registrations in a separate load context with AI core shared, including ambient invocation-name and HTTP client-IP behavior. Cover version-tolerant identity matching and different assembly-name/key lookalikes separately. A host/SDK upgrade that changes the inventory requires compatibility review; absence of a known type is not proof that its replacement is safe.
